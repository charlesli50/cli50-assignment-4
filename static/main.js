...
function displayChart(data) {
  ...
  // Access the canvas element in the HTML
  var ctx = document.getElementById("similarity-chart").getContext("2d");
  var similarityChart = new Chart(ctx, {
    type: "bar",
    data: {
      labels: data.indices,
      datasets: [
        ...
        tooltip: {
          callbacks: {
            title: function (tooltipItem) {
              ...
              return `Document ${data.indices[index]}`;
            },
            label: function (tooltipItem) {
              var index = tooltipItem.dataIndex;
              return `Similarity: ${data.similarities[index].toFixed(4)}`;
            },
          },
        },
      },
